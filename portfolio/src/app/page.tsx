...
const FINESSE_PLACEHOLDER: ProjectItem = {
  id: 'finesse-plugin',
  title: 'Finesse Plugin',
  description:
    'A Claude Code plugin for autonomous development workflow planning and execution. Orchestrates multi-agent validation, prompt construction, and iterative development loops.',
  technologies: [],
  link: '',
  image: '',
  tags: ['Featured', 'AI'],
...
function Decorations() {
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
      <div className="absolute top-8 left-7 w-12 h-12 rounded-xl bg-indigo-200/30 rotate-6" />
      <div className="absolute top-8 left-24 w-9 h-9 rounded-lg bg-rose-200/25 -rotate-3" />
      <div className="absolute top-24 left-10 w-6 h-6 rounded-md bg-amber-200/30 rotate-12" />
      <div className="absolute bottom-28 left-7 w-14 h-8 rounded-xl bg-emerald-200/25 -rotate-2" />
      <div className="absolute top-[38%] left-5 w-4 h-4 rounded bg-violet-200/25 rotate-45" />
    </div>
  );
}
  ...
  switch (sectionId) {
    ...
    case 'projects':
      return (
        ...
        >
          <FeaturedItem
            title={FINESSE_PLACEHOLDER.title}
            subtitle=""
            meta=""
            description={FINESSE_PLACEHOLDER.description}
            tags={FINESSE_PLACEHOLDER.tags}
            isWip
          />
        </SectionFace>
      );
    case 'extras':
      return (
        <SectionFace
          ...
          title="Leadership"
          onExpand={onExpand!}
          itemCount={extracurriculars.length + education.length}
        >
          <FeaturedItem
